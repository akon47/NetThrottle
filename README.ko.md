...
- 규칙별 실시간 업로드/다운로드 속도를 1초마다 표시합니다.
- 엔진을 실행 중인 상태에서도 규칙을 추가/수정/삭제할 수 있습니다.
...
1. NetThrottle을 **관리자 권한**으로 실행합니다(앱이 자동으로 권한 상승을 요청합니다).
2. **Add rule**을 눌러 새 규칙을 추가합니다.
3. **Process (image name)** 열에서 프로세스를 선택하거나 이미지 이름(예: `chrome.exe`)을 직접 입력합니다. **Refresh apps**로 실행 중인 프로세스 목록을 새로 고칠 수 있습니다.
4. **Protocol** 열에서 `Tcp`, `Udp`, `Both` 중 하나를 고릅니다.
5. **Down KB/s**와 **Up KB/s**에 한도를 KB/s 단위로 입력합니다. `0`은 해당 방향 무제한을 뜻합니다.
6. **On** 체크박스로 규칙을 켜고, 툴바의 **Start**(▶)를 눌러 엔진을 시작합니다. 시작하면 버튼이 **Stop**(■)으로 바뀝니다.
7. 각 규칙 행의 **↓ live** / **↑ live** 열에서 실시간 다운로드/업로드 속도를 확인할 수 있습니다.
