...
<p align="center">
  <img src="docs/screenshot.png" alt="NetThrottle 메인 화면" width="880">
</p>
...
- 전체 업/다운 합계 표시와 "Limited only"(제한된 항목만) 필터를 제공합니다.
- 창을 닫으면 알림 영역(트레이)으로 최소화되며, 백그라운드에서 제한이 계속 동작합니다.
- OS 언어를 따르는 다국어 UI(영어·한국어 기본 포함). 실행 파일 옆 `locales/` 폴더에 JSON 파일만 추가하면 언어를 늘릴 수 있습니다.
- 설정 창에서 언어, **Windows 시작 시 자동 실행**(권한 상승 작업 스케줄러), **트레이로 최소화하여 시작**, **KB/s ⇄ MB/s** 단위 전환을 관리합니다.
- 열 클릭으로 정렬(실시간 사용량순으로 대역폭 많이 쓰는 앱 찾기)하고, 창 크기/위치를 기억합니다.
